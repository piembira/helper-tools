...
class Debug
{
    ...
    private static function _processString(array $matches)
    {
        ...
        return '<span style="color: #082A4D;">string</span>(<span style="color: #1287DB;">' . $matches['length'] . '</span>) <span style="color: #787878;">' . $matches['value'] . '</span>';
    }
    ...
    private static function _processArray(array $matches)
    {
        ...
        $key = '<span style="color: #7CAD44;">"' . $matches['key'] . '"</span>';
        $class = '';
        $scope = '';
        ...
        if (isset($matches['scope']) && !empty($matches['scope'])) {
            $scope = ':<span style="color: #787878;">' . $matches['scope'] . '</span>';
        }
    ...
    {
        $type = '<span style="color: #082A4D;">' . $matches['type'] . '</span>';
    ...
    {
        return '<span style="color: #082A4D;">bool</span>(<span style="color: #082A4D;">' . $matches['value'] . '</span>)';
    }
    ...
    {
        return '<span style="color: #082A4D;">float</span>(<span style="color: #1287DB;">' . $matches['value'] . '</span>)';
    }
    ...
    {
        return '<span style="color: #082A4D;">resource</span>(<span style="color: #3574A9;">' . $matches['count'] . '</span>) of type (<span style="color: #4D5D94;">' . $matches['class'] . '</span>)';
    }
    ...
    {
        return '<span style="color: #082A4D;">object</span>(<span style="color: #4D5D94;">' . $matches['class'] . '</span>)#' . $matches['id'] . ' (<span style="color: #3574A9;">' . $matches['count'] . '</span>)';
    }
